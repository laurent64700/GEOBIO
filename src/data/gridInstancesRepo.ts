...
// Deliberately NOT routed through undoableWrite, unlike updateGridInstanceOrigin
// below — same reasoning as missionsRepo.ts's deleteMission: this removes an
// entire grid (and, via the DB's on-delete-cascade FK, every one of its
// grid_line rows) in one irreversible action, not a single fine-grained edit
// a user would expect Annuler to step back through. UI callers must confirm
// before calling this (ConfirmDialog, same pattern as mission deletion).
// Exists to let Laurent clean up a grid generated by mistake or more than
// once for the same network — until now there was no way to ever remove one,
// so a duplicate ("Ajouter une grille" clicked twice) was permanent (field
// testing 08/2026: "attention au lignes doubles curry et doubles hartmann").
export async function deleteGridInstance(id: string): Promise<void> {
  await cachedWrite('grid_instance', 'grid_instance', 'delete', { id }, () => ({ id }), async () => {
    const { error } = await supabase.from('grid_instance').delete().eq('id', id)
    if (error) throw new SupabaseQueryError(`Impossible de supprimer la grille : ${error.message}`)
    return { id }
  })
}
