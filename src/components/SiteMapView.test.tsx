...
import { SiteMapView, type SiteMapViewProps } from './SiteMapView'
...
// editMode/calquesOpen are now controlled props (Task 13 lifted them into
// MissionWorkspace) — most tests below never toggle either one, so a static
// `false` + no-op callback is fine. But the handful of tests that actually
// click "Mode édition" (or, in the future, a Calques toggle) need the click
// to really flip state and re-render, the way MissionWorkspace's real
// setEditMode/setCalquesOpen would — a no-op callback leaves SiteMapView
// re-rendered with the SAME false it was given, so nothing gated by edit
// mode ever appears. This wrapper owns real state for exactly those tests.
function ControlledSiteMapView(
  props: Omit<SiteMapViewProps, 'editMode' | 'onEditModeChange' | 'calquesOpen' | 'onCalquesOpenChange'>
) {
